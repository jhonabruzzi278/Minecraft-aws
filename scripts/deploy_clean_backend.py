...
# Código completo, limpio y optimizado para la EC2 privada
...
def get_usuarios():
    if USE_DYNAMODB and dynamo_table:
        try:
            ...
            # Convertir decimales a int si existen
            for it in items:
...
print("Escribiendo código actualizado en la instancia backend...")
...
time.sleep(2)
# Test rápido de comando RCON
r = requests.post("http://18.215.185.57:8081/api/minecraft/command", json={"comando": "whitelist list"})
print("\n[RCON TEST - Whitelist List]:", r.status_code, r.json())

# Test de creación de usuario y auto-whitelist
r2 = requests.post("http://18.215.185.57:8081/api/usuarios", json={
    "nombre": "Gamer Duoc",
    "email": "gamer@duoc.cl",
    "gamertag": "GamerDuoc2026"
})
print("\n[USER + RCON WHITELIST TEST]:", r2.status_code, r2.json())
